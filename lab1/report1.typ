...
All Q-point node voltages were recorded and verified, as well as verifying that $V_(\B\E) = +0.7V$. The DC blocking of the capacitors was also verified. From the data, $I_(\C\Q)$ and $V_(\C\E)$ were calculated. These were then compared to the prelab results. $beta$ was calculated using the measured and calculated voltages, and then compared to the measuread $beta$ from the multimeters $\h_"fe"$ measurement mode.
